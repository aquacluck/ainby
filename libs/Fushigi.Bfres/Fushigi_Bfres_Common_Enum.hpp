...
namespace Fushigi_Bfres::Common::Enum {
    enum class BfresIndexFormat: uint32_t {
        UnsignedByte = 0,
        UInt16 = 1,
        UInt32 = 2,
    };

    enum class BfresPrimitiveType: uint32_t {
        Triangles = 0x03,
        TriangleStrip = 0x04,
    };

    enum class BfresAttribFormat: uint32_t {
        // 8 bits (8 x 1)
    ...
    enum class MaxAnisotropic: uint8_t {
        Ratio_1_1 = 0x1,
        Ratio_2_1 = 0x2,
        Ratio_4_1 = 0x4,
        Ratio_8_1 = 0x8,
    ...
    enum class TexBorderType: uint8_t {
        White,
        Transparent,
        Opaque,
    };

    enum class TexWrap: int8_t {
        Repeat,
        Mirror,
        Clamp,
        ClampToEdge,
    ...
    // FIXME these aren't powers of 2, but it had [Flags]? uint8_t is also a guess
